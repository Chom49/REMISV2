...
<div class="space-y-6">
    ...
    {{-- ── FO Recommendation Banner ────────────────────────────── --}}
    @if(!empty($showFoRecommendation) || session('fo_recommendation'))
    <div id="fo-recommendation-banner"
         class="relative overflow-hidden rounded-2xl border border-indigo-300 bg-gradient-to-br from-indigo-600 to-purple-700 p-6 shadow-md">
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="relative inline-block">
                                ...
                                <button onclick="toggleDropdown(this)"
                                    ...
                                    Actions
                                    <svg class="w-3.5 h-3.5 text-gray-500 transition-transform duration-150 dropdown-arrow"
                                         fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                {{-- Dropdown panel --}}
                                <div class="dropdown-panel absolute right-0 top-full mt-2 w-56 bg-white rounded-xl
                                    ...
                                    {{-- Edit Profile — always shown for all tenants --}}
...
function toggleDropdown(btn) {
    const panel = btn.closest('.relative').querySelector('.dropdown-panel');
    const arrow = btn.querySelector('.dropdown-arrow');
    const isOpen = panel.style.display === 'block';

    // Close all other open dropdowns first
    document.querySelectorAll('.dropdown-panel').forEach(p => p.style.display = 'none');
    document.querySelectorAll('.dropdown-arrow').forEach(a => a.style.transform = '');
